...
// @ts-nocheck -- The DSP body below is a mechanical migration of the frozen legacy dj-analyzer.js algorithm.
import { analyzerUnavailable, DjBeatmapError, normalizeDjBeatmapError } from './errors'
import type { PodcastDjAnalyzer } from './types'

export const DJ_ANALYZER_LIMITS = Object.freeze({
  headTimeoutMs: 10_000,
  downloadTimeoutMs: 90_000,
  rangeMaxBytes: 48 * 1024 * 1024,
  introMaxBytes: 64 * 1024 * 1024,
  fullMaxBytes: 384 * 1024 * 1024,
})

async function defaultDecoderLoader() {
  try {
    // Variable import keeps the legacy decoder optional: no new package dependency is introduced.
    const packageName = 'mpg123-decoder'
    const loaded = await import(packageName)
    if (!loaded || typeof loaded.MPEGDecoder !== 'function') throw new Error('Decoder unavailable')
    return loaded.MPEGDecoder
  } catch (error) {
    throw analyzerUnavailable(error)
  }
}

async function createMpegDecoder(opts) {
  try {
    const load = opts.decoderLoader || defaultDecoderLoader
    const MPEGDecoder = await load()
    const decoder = new MPEGDecoder({ enableGapless: false })
    await decoder.ready
    return decoder
  } catch (error) {
    if (error instanceof DjBeatmapError) throw error
    throw analyzerUnavailable(error)
  }
}

async function openAudioResponse(audioUrl, init, opts, maxBytes, timeoutMs, enforceContentLength = true) {
  const fetchImpl = opts.fetchImpl || globalThis.fetch
  if (typeof fetchImpl !== 'function') throw analyzerUnavailable(new Error('fetch unavailable'))
  const timeout = Math.max(1, Number(timeoutMs) || DJ_ANALYZER_LIMITS.downloadTimeoutMs)
  try {
    ...
    if (enforceContentLength && maxBytes > 0 && contentLength > maxBytes) {
      try { await response.body?.cancel() } catch { /* best-effort cancellation */ }
      throw new DjBeatmapError('DJ_UPSTREAM_TOO_LARGE', { status: 413 })
    }
...
async function countDownloadedBytes(reader, downloadedBytes, value, maxBytes) {
  const next = downloadedBytes + Number(value?.byteLength || value?.length || 0)
...
async function decodePodcastDjEnergyRange(audioUrl, opts) {
  opts = opts || {};
  const decoder = await createMpegDecoder(opts);
  ...
  try {
    ...
    const opened = await openAudioResponse(
      audioUrl,
      { headers },
      opts,
      opts.limitSec ? DJ_ANALYZER_LIMITS.introMaxBytes : DJ_ANALYZER_LIMITS.rangeMaxBytes,
      DJ_ANALYZER_LIMITS.downloadTimeoutMs,
      !opts.limitSec,
    );
    ...
    while (true) {
      ...
      downloadedBytes = await countDownloadedBytes(reader, downloadedBytes, value, opened.maxBytes);
      chunks++;
...
async function analyzePodcastDjIntro(audioUrl, opts) {
  ...
  const decoded = await decodePodcastDjEnergyRange(audioUrl, {
    ...
    decoderLoader: opts.decoderLoader,
    limitSec: introSec + 8,
  });
...
async function analyzePodcastDjRangeSamples(audioUrl, opts) {
  ...
  try {
    const opened = await openAudioResponse(
      ...
      opts,
      0,
      DJ_ANALYZER_LIMITS.headTimeoutMs,
      false,
    );
    contentLength = Number(opened.response.headers.get('content-length') || 0) || 0;
  } catch {
    contentLength = 0;
  }
  if (!contentLength) {
  ...
  const globalStep = clampRange(median(stepVotes) || sampleMaps[0].map.gridStep || 0.50, 0.32, 0.86);
...
async function analyzePodcastDjStream(audioUrl, opts) {
  ...
  if (durationSec > 3300 && durationSec <= FULL_STREAM_QUALITY_LIMIT_SEC) {
    try {
      ...
      console.warn('[PodcastDjBeatmap] full-stream quality path failed, falling back to range:', normalizeDjBeatmapError(err).code);
...
async function analyzePodcastDjStreamFull(audioUrl, opts) {
  ...
  try {
    const opened = await openAudioResponse(
      ...
      opts,
      DJ_ANALYZER_LIMITS.fullMaxBytes,
      DJ_ANALYZER_LIMITS.downloadTimeoutMs,
    );
    const resp = opened.response;
...
export interface PodcastDjAnalyzerDependencies {
  fetchImpl?: typeof fetch;
  decoderLoader?: () => Promise<new (options: { enableGapless: boolean }) => any>;
}

export function createPodcastDjAnalyzer(dependencies: PodcastDjAnalyzerDependencies = {}): PodcastDjAnalyzer {
  return {
    analyzeStream(audioUrl, options) {
      return analyzePodcastDjStream(audioUrl, { ...options, ...dependencies });
    },
    analyzeIntro(audioUrl, options) {
      return analyzePodcastDjIntro(audioUrl, { ...options, ...dependencies });
    },
  };
}
