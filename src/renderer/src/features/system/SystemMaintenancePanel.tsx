  ...
  return (
    <section className={`system-maintenance-panel ${className}`.trim()} aria-label="应用更新">
      <header className="system-maintenance-heading">
        <div>
          <span className="system-maintenance-eyebrow">FLUXPLAYER</span>
          <h2>应用更新</h2>
        </div>
      ...
      <div className="system-maintenance-grid system-maintenance-grid--single">
        <GlassSurface className="system-maintenance-card">
          <header className="system-maintenance-card-header">
            <div>
              <span className="system-maintenance-card-index">01</span>
              <h3>版本与更新</h3>
            </div>
            <span className="system-maintenance-status" data-tone={copy.tone}>
              <span />
              {copy.label}
            </span>
          </header>
          ...
          <dl className="system-maintenance-metadata">
            <div>
              <dt>当前版本</dt>
              <dd>{version(state.currentVersion)}</dd>
            </div>
            <div>
              <dt>可用版本</dt>
              <dd>{version(state.availableVersion)}</dd>
            </div>
          </dl>
          {state.progress ? (
            <div className="system-maintenance-progress">
              <div className="system-maintenance-progress-heading">
                <strong>下载进度</strong>
                <span>{percent.toFixed(1)}%</span>
              </div>
              <div className="system-maintenance-progress-track">
                <span style={{ width: `${percent}%` }} />
              </div>
            </div>
          ) : null}
          {state.error ? (
            <div className="system-maintenance-message system-maintenance-message--danger">
          ...
          <footer className="system-maintenance-actions system-maintenance-actions--updater">
            <button
              className="system-maintenance-button"
            ...
            <button
              className="system-maintenance-button system-maintenance-button--primary"
