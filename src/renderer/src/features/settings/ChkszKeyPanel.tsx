...
export function ChkszKeyPanel({ className = '' }: { className?: string }): React.JSX.Element {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      const { configured: hasKey } = await desktop.chksz.getStatus()
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setKeyInput('')
      setSaveState('saved')
      showToast('ChKSz 已接入，直连无音源时自动兜底', {
        title: 'ChKSz 已接入',
        tone: 'info',
        duration: 5000,
      })
      ...
      <div className="grid grid-cols-1 gap-[0.9rem]">
        ...
        >
          ...
          <p className="text-[0.72rem] leading-[1.55] text-[var(--flux-text-muted)]">
            {configured
              ? '各平台优先走账号直连（会员音质）。无音源或仅试听时自动通过 ChKSz 兜底解析。密钥仅本机加密存储，不会上传。'
              : '访问 api.chksz.com 注册并获取以 chksz_ 开头的个人 API Key。配入后各平台直连无音源或仅试听时自动兜底解析。'}
          </p>
