...
describe('网易云 fixture 快照', () => {
  it('cloudsearch result.songs → mapSongRecord：不变量 + 快照', async () => {
    ...
    expect(Array.isArray(songs) && songs.length > 0, `上游形状漂移：result.songs 缺失或为空（${cloudsearch.meta.endpoint}）`).toBe(true)
    ...
    await expect(JSON.stringify(mapped, null, 2)).toMatchFileSnapshot('./__snapshots__/netease-search-songs.mapped.json')
  })

  it('song_detail songs → mapSongRecord：固定 id 186016，全量快照', async () => {
    const songs = detail.response && detail.response.songs
    expect(Array.isArray(songs) && songs.length > 0, `上游形状漂移：songs 缺失或为空（${detail.meta.endpoint}）`).toBe(true)
    ...
    await expect(JSON.stringify(song, null, 2)).toMatchFileSnapshot('./__snapshots__/netease-detail-song.mapped.json')
  })
  ...
  it('playlist_track_all songs → mapSongRecord：不变量 + 快照', async () => {
    ...
    expect(Array.isArray(songs) && songs.length > 0, `上游形状漂移：songs/tracks 缺失或为空（${playlist.meta.endpoint}）`).toBe(true)
    const mapped = (songs as any[]).map(mapSongRecord).map(normCover)
    for (const s of mapped) {
